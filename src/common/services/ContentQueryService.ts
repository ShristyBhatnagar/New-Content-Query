...
export class ContentQueryService implements IContentQueryService {
  ...
  public generateDefaultTemplate(viewFields: string[], itemSelectorEnabled: boolean): string {
    let viewFieldsStr = viewFields.map((field) => { return Text.format("                    <span class='ms-DetailsRow-cell' style='display:flex;flex-direction:column; width:200px; text-align:center; '><strong>{0} </strong>\{\{{0}.textValue\}\}</span>", field); }).join("\n");
    ...
    let template = Text.format(`<style type="text/css">
      ...
      .dynamic-template::-webkit-scrollbar {
        display: none;
      }
  ...
  <div style='display:flex; position:sticky; background-color: white;
  top: 0;'>
  <strong style='display:flex;  text-align:center;'>${viewFieldsStr} </strong>
  </div>
          ...
          {{#each items}}
              <div class="dynamic-item ">
                  ...
                  <div style='display:flex;'>
                      <div class="difTitle" style='display:flex;     border-bottom: 1px  solid #E8E8E8;
                      padding-bottom: 10px;'>
                        {1}
                      </div>
                      <div class="difBody">
